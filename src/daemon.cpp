...
struct command_t {
  enum class TYPE { GET_PING, GET_STATISTIC };
  ...
  void UnixSocketListen() {
    ...
    while (_run) {
      ...
      std::string command_type =
          (command.type == command_t::TYPE::GET_PING ? "Ping"
                                                     : "Get statistic");
      spdlog::info("Accept new command {}", command_type);

      std::shared_ptr<ICommandHandler> handler =
          std::shared_ptr<ICommandHandler>(
              handlerFabric.createHandler(command););

      if (command_type == command_t::TYPE::GET_PING) { // SWAP TO THREAD_POOL
        std::thread worker([&]() { handler->process(command); });
        worker.detach();
      } else {
